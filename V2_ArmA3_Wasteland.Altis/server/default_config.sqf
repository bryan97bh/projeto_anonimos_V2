...
A3W_artilleryRadius = 75;          // Strike radius in meters within which the shells will randomly hit
...
A3W_remoteBombStoreRadius = 10;    // Prevent players from placing any kind of explosive on the ground within this distance from any store (0 = disabled)
...
A3W_vehiclePurchaseCooldown = 3;  // Number of seconds to wait before allowing someone to purchase another vehicle, don't bother setting it too high because it can be bypassed by rejoining
...
A3W_mineLifetime = 0*24;           // Maximum lifetime in hours for saved mines across server restarts (0 = no time limit)

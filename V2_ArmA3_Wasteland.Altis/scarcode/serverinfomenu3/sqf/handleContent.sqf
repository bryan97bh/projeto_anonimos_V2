...
if not ( isNull _dsp ) then
{
   ...
   _ctrlContentTitle ctrlSetStructuredText parseText ( getText ( missionConfigFile >> "CfgServerInfoMenu3" >> "menuItems" >> _cn >> "title" ) );
   _ctrlContent = _dsp displayCtrl 9;
   _ctrlContent ctrlSetStructuredText parseText "";
   _ca = getArray ( missionConfigFile >> "CfgServerInfoMenu3" >> "menuItems" >> _cn >> "content" );
   _cntnt = "";
   { // Build the content
      _cntnt = _cntnt + _x;
   } forEach _ca;
